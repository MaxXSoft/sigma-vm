...
pub enum TokenKind {
  /// Line number generated by the C preprocessor.
  #[regex(r#"# [0-9]+ "<stdin>"\n"#)]
  LineNum(LineNum),
  /// Spaces and comments.
  #[skip(r"\s+|#.*\n|#.*")]
...
impl str::FromStr for LineNum {
  type Err = <u32 as str::FromStr>::Err;

  fn from_str(s: &str) -> Result<Self, Self::Err> {
    let s = s.trim_matches(|c| "# \"<stdin>\n".contains(c));
    s.parse().map(|line_num| Self { line_num })
  }
}
...
token_ast! {
  ...
  pub macro Token<TokenKind> {
    [line_num] => { kind: TokenKind::LineNum(_), prompt: "line number" },
...
impl<R> Parser<R> {
  ...
  where
    R: io::Read,
  {
    match self.tokens.parse::<StatementOrEof>()? {
      StatementOrEof::LineNum(l) => {
        self
          .tokens
          .inner_mut()
          .input_mut()
          .set_line_col(l.unwrap::<LineNum, _>().line_num, 0);
        self.parse()
      }
      StatementOrEof::Statement(stmt) => Ok(Some(stmt)),
      StatementOrEof::Eof(_) => Ok(None),
    }
  }
}
