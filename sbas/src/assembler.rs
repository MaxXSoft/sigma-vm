...
pub struct Assembler {
  ...
  pending_const_labels: HashMap<String, Vec<u64>>,
}
...
impl Assembler {
  ...
  pub fn write<W>(self, span: Span, w: &mut W) -> Result<()>
  where
    W: Write,
  {
    // check if there are undefined labels
    for (name, info) in self.labels.into_iter() {
    ...
    for (id, info) in self.temp_labels.into_iter() {
    ...
    match label_ref {
      LabelRef::Named(l) => match self.labels.get(l.unwrap_ref::<&String, _>()) {
        Some(LabelInfo { kind, .. }) => match kind {
          LabelKind::Const(index) => Ok(*index),
...
/// Kind of label.
enum LabelKind {
  /// Constant label, with the constant index.
  Const(u64),
